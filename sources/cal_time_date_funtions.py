...
def current_datetime():
    ...
    return f'Date : <b class="text-2xl">{current_date}</b><br>Time : <b class="text-2xl">{current_time}</b>'

def current_date():
    # date
    current_date = str(datetime.datetime.now().date())
    return f'Today Date : <b class="text-2xl">{current_date}</b>' 

def current_time():
    # time
    current_time = str(datetime.datetime.now().time())
    return f'Current Time : <b class="text-2xl">{current_time}</b>' 

def current_date_all():
    # date all

    current_date = datetime.date.today()
    return f"""Current date (YYYY-MM-DD) <br> <b class="text-2xl">{current_date.strftime('%Y-%m-%d')}</b><br>
       Current date (MM/DD/YYYY): <br> <b class="text-2xl">{current_date.strftime('%m/%d/%Y')}</b><br>
       Current date (DD-MMM-YY): <br> <b class="text-2xl">{current_date.strftime('%d-%b-%y')}</b><br>
       Current date (Month Day, Year): <br> <b class="text-2xl">{current_date.strftime('%B %d, %Y')}</b><br>
       """
...
def current_time_all():
    ...
    return f"Current time (24-hour format) <br> <b class='text-2xl'>{current_time.strftime('%H:%M:%S')}</b><br>" \
       f"Current time (12-hour format) <br> <b class='text-2xl'>{current_time.strftime('%I:%M:%S %p')}</b><br>" \
       f"Current time (Natural) <br> <b class='text-2xl'>{current_time.strftime('%I:%M %p')}</b><br>"
...
def difference_between_dates(year1,month1,date1,year2,month2,date2):
    ...
    try:    
        ...
        return f"differnece between <b>{year1}-{month1}-{date1}</b> and <b>{year2}-{month2}-{date2}</b> is <b class='text-2xl'>{str(date1_-date2_).replace('-', '').split(',')[0]}</b>"
    except:
        return 'If you are trying to get the difference between two dates, you can send a message in the following format: <br> <center>"difference between dates <b>yyyy mm dd</b> and <b>yyyy mm dd</b>"</center> <br>where "<b>yyyy</b>" is the year, "<b>mm</b>" is the month, and "<b>dd</b>" is the day of the month for the dates you want to find the difference between. This message will tell the program to retrieve the difference between the specified dates. <br> <br> However,<br> <span style="color:red;">"I could not find the difference between the given dates because the given date(s) is(are) invalid.</span>'
...
def next_day(day):
    ...
    try:
        ...
        return 'If you are trying to get the next day, you can send a message in the following format: <br> <center>"next <b>name of the day</b></center> <br>where "name of the day will be" monday, tuesday, ....sunday "<b>dd<This message will tell the program to retrieve the next day occurence. <br> <br> However,<br> <span style="color:red;">"I could not find the next day occurence because the given day name is invalid.</span>'
...
def time_in_zone(zone):
    ...
    try:
        ...
        return(f"The current time in <b>{timezone.zone}</b> is <b  class='text-2xl'>{current_time}</b>")
    except:
        return 'If you are trying to get the time in specific timezone, you can send a message in the following format: <br> <center>time for "zone-name" timezone </center> <br>where "<b>zone</b>" is the name of the timezone ex. Asia/Kolkata.. This message will tell the program to retrieve the time for specific timezone. <br> <br> However,<br> <span style="color:red;">"I could not find the time for given timezone because the given timezone is invalid.</span>'
...
def leap(year):
    ...
    try:
        ...
        return 'If you are trying to find the specific year is leap year or not, you can send a message in the following format: <br> <center>check leap yyyy </center> <br>where "<b>yyyy</b>" is the year. This message will tell the program to retrieve the result of leap year for specific year.<br> <br> However,<br> <span style="color:red;">"I could not find the result for given year because the given year is invalid.</span>'
        
        
        
def whole_month(year,month):
    # calendar for month yyyy mm
    
    try:
        cal = calendar.HTMLCalendar()
        html_code = cal.formatmonth(year, month).replace('border="0"', 'border="1"').replace('cellpadding="0"', 'cellpadding="2"').replace('class="month"', 'class="text-xl"').replace('colspan="7"', 'colspan="7" style="text-align:center;" ')
        # print(f'<p class="text-2xl"> {html_code} </p>')
        return html_code
    except:
...
def whole_year(year):
    ...
    try:
        ...
        html_code = ''
        for i in range(1,13):
            html_code += cal.formatmonth(year, i).replace('border="0"', 'border="1"').replace('cellpadding="0"', 'cellpadding="2"').replace('class="month"', 'class="text-xl"').replace('colspan="7"', 'colspan="7" style="text-align:center;" ')+'<br><br>'
        return(html_code)
    except:
